...
The repository contains a private-first Next.js studio and a Git-backed public
wiki alongside the NeoForge mod. The first hosted installation is a
**prototype**: it is suitable for Daniel and Jake to validate the workflow, but
not for public launch or irreplaceable production data.
...
publication bundle -> local deterministic exporter -> data/published + mod JSON
Git pull request -> main -> wiki deployment and Gradle build
```
...
- `src/main/resources/data/cobblemon_kinetics/work_profiles` is generated from
  the same approved snapshot and is never edited by hand.
...
| `src/main/resources/data/.../work_profiles` | Deterministic mod-facing output                                                          |

The pnpm workspace and Gradle project share the repository but remain
independent. A normal `./gradlew build` does not install Node packages, start
Docker, contact Supabase, or build the website.
...
Start the local services and verify both projects independently:
...
5. Review the changed public records, manifest, and generated work profiles in
   a pull request.
6. CI scans Git history for secrets without posting bot comments, then validates
   the JSON Schemas, deterministic hashes, website, database policies, and Java
   parser.
...
The first work-profile format is parsed by the mod on server-data reload. It
accepts only registered code-side workstation adapters. The Hydro profile is a
validated contract in this slice; existing Hydro gameplay still uses its
current config and saved-data path until that migration is tested separately.
